...
namespace cppjieba {
  ...
  public:
    typedef struct _Word {string word; vector<size_t> offsets; double weight;} Word; // 定义单词结构体 Word
  private:
  ...
  public: 
    // 构造函数
    TextRankExtractor(const string& dictPath, const string& hmmFilePath, const string& stopWordPath, const string& userDict = "") : segment_(dictPath, hmmFilePath, userDict) {
      LoadStopWordDict(stopWordPath);
    }

    // 构造函数
    TextRankExtractor(const DictTrie* dictTrie, const HMMModel* model, const string& stopWordPath) : segment_(dictTrie, model) {
      LoadStopWordDict(stopWordPath);
    }
